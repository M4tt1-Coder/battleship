...
/**
 * Represents a server discovered via UDP discovery.
 * This record is used as a lightweight data container for the GUI and connection logic.
 * A discovered server contains:
 * - host: the IP address of the server (taken from the UDP packet sender)
 * - port: the TCP port to connect to (sent by the server in the discovery response)
 * - name: an optional human-readable server name
