 ...
 *
 * Responsibilities:
 * - Parse incoming socket lines into Message objects
 * - Drive NetworkStateMachine
 * - Auto-send mandatory setup acknowledgements (DONE/OK/READY)
 * - Integrate gameplay:
 *   - On SHOT: evaluate on local board, send ANSWER (0/1/2)
 *   - On ANSWER: apply result to opponent board using Game.applyOpponentsResponseToPlayersShot(...)
 *   - On MISS (answer 0): shooter sends PASS to switch turns (protocol example)
 *
 * Coordinate handling:
 * - Protocol uses 1-based row/col (starts at 1)
 * - Internal board uses 0-based indices (arrays)
 *
 * @author WoFabian
 */
public class NetworkGameController implements MessageListener {
    ...
    // Remember last shot we sent (internal 0-based) so we can apply incoming ANSWER to the correct cell.
     ...
     *
     * IMPORTANT:
     * - For PvP network play, game.getPlayingMode() should be VS_PLAYER.
     * - The turn should start as:
     *   - server side: PlayerTurn.PLAYER
     *   - client side: PlayerTurn.OPPONENT
     *
     ...
     *
     * Protocol is 1-based, so we convert (x+1, y+1).
     *
    ...
    /**
     * Auto-ack required by protocol (client side only):
     * - after SIZE -> DONE
     * - after SHIPS -> DONE
     * - after LOAD -> OK
     * - after READY -> READY
     ...
     *
     * Protocol coordinates are 1-based. We convert them to internal 0-based Coordinates.
     *
     * Then we evaluate on our local board:
     * - MISS  -> answer 0
     * - HIT   -> answer 1
     * - SUNK  -> answer 2
     *
     * IMPORTANT:
     * - This requires "game" to be set and in VS_PLAYER mode.
     *
     ...
     *
     * - Applies the result to the opponent board (so our local model knows what happened)
     * - If answer == 0 (MISS), we must send PASS (protocol example) and switch turn
     * - If HIT/SUNK, we continue (no pass)
     *
     ...
     *
     * PASS means the opponent missed and is giving us the turn.
     *
     * @author WoFabian
     */
     ...
     * - 2 = hit + sunk
     *
     * INVALID is not part of protocol; we treat it as MISS (0).
     *
     * @param r result
