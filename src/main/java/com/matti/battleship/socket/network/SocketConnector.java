 ...
 *
 * <p>
 * This class is responsible for sending and receiving single protocol lines
 * over an existing
 * {@link Socket}. It deliberately does not create threads; the caller decides
 * where/when {@link
 * #listenLoop()} runs.
 *
 * <p>
 * LOGIC-IMPORTANT: Listening can be started/stopped without closing the socket
 * (Task 2). This is
 * implemented by using a socket timeout so {@code readLine()} wakes up
 * regularly and the loop can
 * check stop flags.
 *
 * <p>
 * LOGIC-IMPORTANT: Turn logging is handled here (via {@link TurnLog}). The
 * connector derives a
 * "turn" from protocol commands and writes a readable send/receive pairing to
 * the log.
 *
 * @author WoFabian
 */
public class SocketConnector {
  ...
  /**
   * Current logical turn label used by the log (protocol-derived, not a game-rule
   * engine).
   */
  ...
  /**
   * Label for the other side (SERVER or CLIENT) used only for logging/turn
   * formatting.
   */
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: We treat communication as send/receive "pairs" to insert
   * separators and
   * repeat headers only after a pair is complete. This keeps the {@link TurnLog}
   * readable even when
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: {@code pass} switches the turn after the current
   * send/receive pair is
   * completed (logging readability). We delay the header switch until
   * {@link #finishPair()}.
   */
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: The listen loop and GUI/controller calls may run on
   * different threads. We
   * guard reads/writes to {@code listeningEnabled} and {@code stopLoopRequested}
   * to keep Task 2
  ...
  /**
   * Enables/disables message processing inside {@link #listenLoop()} without
   * closing the socket.
   *
   * <p>
   * LOGIC-IMPORTANT: When disabled, the loop stays alive but idles briefly and
   * re-checks flags.
   */
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: The loop exits "soon" because {@code readLine()} wakes up
   * via {@code
   * SO_TIMEOUT}.
   */
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: {@code SO_TIMEOUT} is set so {@code readLine()} does not
   * block forever.
  ...
  /**
   * Sets the callback that receives incoming protocol lines and disconnect
   * events.
   *
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: This updates the {@link TurnLog} and pairing state before
   * writing to the
  ...
  /**
   * Enables processing in {@link #listenLoop()} and clears a previous stop
   * request (Task 2).
   *
   * <p>
   * LOGIC-IMPORTANT: This does not start a thread; it only enables the loop
   * if/when it is
   * running.
   *
   */
  ...
  /**
   * Disables processing and requests {@link #listenLoop()} to exit without
   * closing the socket (Task
   * 2).
   *
   * <p>
   * LOGIC-IMPORTANT: This is intentionally not a disconnect. The TCP socket
   * remains open.
   *
   */
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: This method is intentionally blocking and does not start
   * threads. The
   * caller must run it in a suitable background context.
   *
   * <p>
   * LOGIC-IMPORTANT: A stop request is NOT treated as a disconnect. Only a real
   * remote close
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: This is a hard shutdown and will also end any active
   * {@link #listenLoop()}
  ...
  /**
   * Updates pair state after sending and finishes a pair once both directions
   * were observed.
   */
  ...
  /**
   * Updates pair state after receiving and finishes a pair once both directions
   * were observed.
   */
  ...
  /**
   * Completes the current send/receive pair and formats the {@link TurnLog}
   * boundary.
   *
   * <p>
   * LOGIC-IMPORTANT: A delayed turn switch (pendingTurnSwitch) is applied here so
   * the log header
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: This does not enforce game rules. It only derives a readable
   * turn header
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: Some commands (e.g. {@code answer}) may change the derived
   * "turn" based on
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: Commands are treated case-insensitively to be robust against
   * input
   ...
   *
   * <p>
   * LOGIC-IMPORTANT: Returns {@code -1} if the message is malformed so logging
   * logic can fall
