 ...
 *
 * <p>Typical GUI flow: - User selects a server host (e.g. from discovery list) or uses a default
 * host (e.g. localhost) - GUI calls connect(selectedHost, listener) - After the connection is
 * established, the GUI/gameflow sends initial protocol messages (for example "ready" or waits for
 * server setup messages depending on your protocol flow)
 *
 * <p>Important methods for GUI integration: - connect(host, listener): Establishes the TCP
 * connection and starts listening for messages - send(msg): Sends a protocol message to the server
...
public class ClientConnection {
   ...
   *
   * <p>GUI typically calls this for actions like: - sending "shot r c" when user clicks a cell -
   * sending "pass" when needed by gameflow - sending "ready" during setup handshake
   *
   ...
   *
   * <p>GUI typically calls this when: - leaving the online match screen - returning to main menu -
