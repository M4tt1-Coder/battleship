 ...
 *
 * <p>This class replaces separate ClientConnection/ServerConnection by exposing a unified
 * interface for:
...
public final class GlobalConnector {
     ...
     *
     * <p>LOGIC-IMPORTANT: Role controls which calls are allowed (e.g. acceptClient() only for SERVER).
     */
    public enum Role { NONE, CLIENT, SERVER }
     ...
     *
     * <p>LOGIC-IMPORTANT: Its runLoop is executed externally (thread lifecycle is controlled by the caller).
     */
     ...
     *
     * <p>LOGIC-IMPORTANT: This keeps shared connection state consistent (especially busy flag)
     * and forwards events to the external listener if one is installed.
     */
    private final IMessageListener internalListener = new IMessageListener() {
        @Override
     ...
     * @author WoFabian
     */
    public AtomicBoolean getBusyFlag() { return busy; }
     ...
     *
     * <p>LOGIC-IMPORTANT: Any existing connector/server socket is cleaned up first.
     * This allows re-connecting without needing a new GlobalConnector instance.
     *
     ...
     *
     * <p>LOGIC-IMPORTANT: After accept, {@code busy} is set to true so UDP discovery stops advertising
     * this server while the connection is active.
     *
     ...
     *
     * <p>LOGIC-IMPORTANT: Used internally before switching roles (client/server) so stale sockets
     * and responder threads do not leak across reconnect attempts.
     */
     ...
     *
     * <p>LOGIC-IMPORTANT: This prevents invalid API usage (e.g. calling acceptClient while in CLIENT role).
     *
     ...
     *
     * <p>GUI-IMPORTANT: If this throws, the GUI should treat it as "not connected" and update UI state.
     *
