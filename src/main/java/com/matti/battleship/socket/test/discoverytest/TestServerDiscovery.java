 ...
 *
 * <p>This test starts a server using {@link GlobalConnector}, runs a {@link ServerDiscoveryResponder}
 * in a background thread and accepts a TCP client in another background thread so the main thread
 * can still process console input.
 *
 * <p>LOGIC-IMPORTANT: The discovery responder only answers DISCOVER requests while {@code busy == false}.
 * As soon as a TCP client connects, {@code busy} becomes true and the server disappears from the
 * discovery results.
 *
 * <p>LOGIC-IMPORTANT: This is a minimal echo/ack test (ready -> ready, otherwise -> ok).
 * It is not the full Battleship setup handshake (size/ships/done).
 *
 * @author WoFabian
 */
public class TestServerDiscovery {
     ...
     *
     * <p>GUI-OPTIONAL: Development utility for validating discovery visibility, accept lifecycle,
     * and listening start/stop behavior.
     *
     * @param args unused
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Listener must be installed BEFORE acceptClient so the internal forwarding/busy handling is active immediately.
        ...
        System.out.println("[DISCOVERY] Responder läuft (UDP " + EnvConfig.getPort() + ") – solange busy=false.");
     ...
     * Before starting, we call {@link GlobalConnector#requestStartListening()} so the connector
     * loop is enabled (Task 2).
     *
