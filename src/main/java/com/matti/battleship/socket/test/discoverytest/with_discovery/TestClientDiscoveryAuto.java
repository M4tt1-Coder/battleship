 ...
 * - Discovery scan (same as manual variant):
 *   List<DiscoveredServer> servers = new ClientDiscoveryScanner(EnvConfig.getPort()).discover(600);
 * - If list is NOT empty:
 *   Show list and connect using selected.host()/selected.port()
 * - If list IS empty:
 *   Auto fallback logic:
 *   client.connect("localhost", listener);   (Port via EnvConfig inside ClientConnection)
 *   client.send("ready");                   (optional "proof of connection")
 *
 * What NOT to copy (CLI-only):
 * - Console command parsing, while-loops, "exit" handling
 *
 * Purpose of this test:
 * - Shows how the GUI could behave if you want "auto-connect to localhost" when no server is found.
 *
 * @author WoFabian
 */
