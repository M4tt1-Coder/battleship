 ...
 *
 * <p>GUI-OPTIONAL: This is a development utility. It allows testing discovery and basic
 * connectivity without running the full GUI.
 *
 * @author WoFabian
 */
public class TestClientDiscoveryManuell {
    ...
    while (true) {
      ...
      if (parts.length == 2 && parts[1].equalsIgnoreCase("verbinden")) {
        try {
          ...
          if (chosen.port() != port) {
            System.out.println(
                "[WARN] Discovered TCP-Port ("
                    + chosen.port()
                    + ") != EnvConfig PORT ("
                    + port
                    + "). Verbindung nutzt EnvConfig PORT.");
          }
   ...
   *
   * <p>GUI-IMPORTANT: The receive loop is started in a background thread because listenLoop() is
   * blocking.
   *
